...
def collect_files():
    ...
    while running:
        ...
        try:
            ...
            if has_new_files:
                # 查找目标目录中修改时间最新的文件，并提取其序号赋值给 last_number
                latest_file = None
                latest_mtime = 0
                for root_dir, _, files in os.walk(target_dir):
                    for file in files:
                ...
                if latest_file:
                    try:
                        # 从文件名中提取序号
                        last_number = int(latest_file.split("_")[1])
                    except (IndexError, ValueError):
                        logging.error(f"无法从文件名 {latest_file} 中提取序号，保持 last_number 不变")
                else:
                    logging.error("未找到最新的日志文件，保持 last_number 不变")
